...
class AuthHelper {
  ...
  _buildHeader(authParts) {
    return [this._config.algoPrefix, 'HMAC', this._config.hashAlgo].join('-') +
      ...
      ', SignedHeaders=' + Utils.formatSignedHeaders(authParts.signedHeaders) +
  ...
  generatePreSignedUrl(requestUrl, expires) {
    ...
    const params = {
      ...
      Expires: expires,
      SignedHeaders: Utils.formatSignedHeaders(new Canonicalizer().getCanonicalizedSignedHeaders(requestOptions.headers,
        headersToSign))
    };
  ...
  parseAuthHeader(authHeader, requestDate, keyDB) {
    const regex = new RegExp('^' + this._algoRegExp() + ' Credential=' + credentialRegExpDefinition + ', SignedHeaders=' +
      signedHeadersRegExpDefinition + ', Signature=' + signatureRegExpDefinition + '$'
    );
