...
var AuthHelper = function(config) {
  ...
  function buildAuthParts(requestOptions, requestBody, headersToSign) {
    ...
    return {
      shortDate: utils.toShortDate(config.date),
  ...
  function generatePreSignedUrl(requestUrl, expires) {
    ...
    var params = {
      ...
      Date: utils.toLongDate(config.date),
      Expires: expires,
  ...
  function generateFullCredentials() {
    return [config.accessKeyId, utils.toShortDate(config.date), config.credentialScope].join('/');
  }
