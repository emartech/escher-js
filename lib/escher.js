...
var Escher = function(configToMerge) {
  ...
  if (config.date && !(config.date instanceof Date)) {
...
Escher.prototype = {
  preSignUrl: function(url, expires) {
    var currentDate = this._config.date || new Date();
  ...
  authenticate: function(request, keyDB, mandatorySignedHeaders) {
    ...
    if (!cryptiles.fixedTimeComparison(parsedAuthParts.config.credentialScope, this._config.credentialScope)) {
    ...
    if (!cryptiles.fixedTimeComparison(parsedAuthParts.shortDate, utils.toShortDate(requestDate))) {
    ...
    if (!cryptiles.fixedTimeComparison(parsedAuthParts.signature, generatedAuthParts.signature)) {
