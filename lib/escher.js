...
Escher.prototype = {
  ...
  signRequest: function(requestOptions, body, headersToSign) {
    ...
    var formattedDate = (this._config.dateHeaderName.toLowerCase() === 'date' ? escherUtil.toHeaderDateFormat(this._config
      .date) : escherUtil.toLongDate(this._config.date));
    var defaultHeaders = escherUtil.normalizeHeaders([
      [this._config.dateHeaderName, formattedDate]
    ]);
    escherUtil.addDefaultHeaders(defaultHeaders, requestOptions);
  ...
  authenticate: function(request, keyDB) {
    ...
    if (isPresignedUrl) {
      ...
      var canonicalizedQueryString = new Canonicalizer().canonicalizeQuery(escherUtil.filterKeysFrom(uri.query, [
        this._queryParamKey('Signature')
      ]));
      request.url = uri.pathname + (canonicalizedQueryString ? '?' + canonicalizedQueryString : '');
    } else {
      requestDate = this._config.dateHeaderName.toLowerCase() === 'date' ? new Date(escherUtil.getHeader(headers,
        this._config.dateHeaderName)) : escherUtil.parseLongDate(escherUtil.getHeader(headers, this._config.dateHeaderName));
      parsedAuthParts = new AuthHelper(this._config).parseAuthHeader(escherUtil.getHeader(headers, this._config.authHeaderName),
        requestDate, keyDB);
    ...
    if (!request.host) {
      request.host = escherUtil.getHeader(headers, 'host');
    }
