...
var Escher = function (configToMerge) {
    ...
    function validateRequest(request, keyDB) {
        ...
        if (isPresignedUrl) {
            requestDate = escherUtil.parseLongDate(uri.query[queryParamKey('Date')]);
            parsedAuthParts = new AuthHelper(config).parseFromQuery(uri.query, requestDate, keyDB);
            requestBody = 'UNSIGNED-PAYLOAD';
            expires = parseInt(uri.query[queryParamKey('Expires')]);
            ...
            requestDate = config.dateHeaderName.toLowerCase() == 'date' ? new Date(getHeader(headers, config.dateHeaderName)) : escherUtil.parseLongDate(getHeader(headers, config.dateHeaderName));
        ...
        if (!isDateWithinRange(requestTime, currentTime, expires)) {
    ...
    function isDateWithinRange(requestTime, currentTime, expires) {
        return (requestTime - config.clockSkew * 1000 <= currentTime)
