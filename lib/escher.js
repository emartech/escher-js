...
Escher.prototype = {
  ...
  authenticate: function(request, keyDB) {
    ...
    if (isPresignedUrl) {
      ...
      requestDate = this._config.dateHeaderName.toLowerCase() === 'date' ? new Date(getHeader(headers, this._config
        .dateHeaderName)) : escherUtil.parseLongDate(getHeader(headers, this._config.dateHeaderName));
      parsedAuthParts = new AuthHelper(this._config).parseAuthHeader(getHeader(headers, this._config.authHeaderName),
        requestDate, keyDB);
