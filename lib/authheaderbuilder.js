...
var AuthHeaderBuilder = function (signerConfig) {
    ...
    function generatePreSignedUrl(requestUrl, expires) {
        var parsedUrl = url.parse(requestUrl, true); // TODO apply fixed parse here too (?)
        var requestOptions = {
            host: parsedUrl.host,
            method: 'GET',
            uri: parsedUrl.path,
        ...
        var params = createPreSignParams(expires, requestOptions);
        Object.keys(params).forEach(function (key) {
            requestUrl = appendQueryParamToUrl(requestUrl, getParamKey(key), params[key]);
        });

        requestOptions.uri = url.parse(requestUrl, true).path;
        var signer = new Signer(signerConfig);
        var signature = signer.calculateSignature(signer.getStringToSign(requestOptions, 'UNSIGNED-PAYLOAD'), signer.calculateSigningKey());
        return  appendQueryParamToUrl(requestUrl, getParamKey('Signature'), signature);
    }

    function createPreSignParams(expires, requestOptions) {
        return {
            ...
            Expires: expires,
            SignedHeaders: formatSignedHeaders(new Canonicalizer().getCanonicalizedSignedHeaders(requestOptions))
        };
    }
