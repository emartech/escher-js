...
var AuthHeaderBuilder = function (config) {
    ...
    function buildHeader(authParts) {
        return config.vendorPrefix + '-HMAC-' + config.hashAlgo.toUpperCase() +
            ' Credential=' + generateFullCredentials(authParts.shortDate) +
    ...
    function createPreSignParams(expires, requestOptions, headersToSign) {
        return {
            Algorithm: [config.vendorPrefix, 'HMAC', config.hashAlgo.toUpperCase()].join('-'),
            Credentials: generateFullCredentials(escherUtil.toShortDate(config.date)),
    ...
    function generateFullCredentials(shortDate) {
        return [config.accessKeyId, shortDate, config.credentialScope].join('/');
    }
    ...
    function algoRegExp() {
        return config.vendorPrefix.toUpperCase() + "-HMAC-([A-Z0-9\\,]+)";
    }
