...
const Canonicalizer = function(hashAlgo) {
  ...
  function canonicalizeRequest(requestOptions, body, headersToSign) {
    const parsedUrl = utils.parseUrl(prepareUri(requestOptions.url), true);
    const headers = filterHeaders(utils.normalizeHeaders(requestOptions.headers), headersToSign);
    const lines = [
  ...
  function getCanonicalizedSignedHeaders(headers, headersToSign) {
    const normalizedHeaders = filterHeaders(utils.normalizeHeaders(headers), headersToSign);
